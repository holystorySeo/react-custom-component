...
export const AutoComplete = () => {
  ...
  const handleKeyPress = (e) => {
    const maxId = matchedList.length;

    if(e.key === 'ArrowDown') {
      if(isIdxSelected === undefined) { //맨처음 키보드 down 할 때 드랍다운 리스트 인덱스 0번을 선택한다.
        setIsIdxSelected(0);
      } else if(isIdxSelected !== undefined) { // 두번째 키보드 down 부터
        if(maxId === isIdxSelected + 1) { // maxId와 isIdxSelected 값의 차이가 1일 경우 인덱스는 0을 설정한다.
          setIsIdxSelected(0); // isIdxSelected의 값은 0이 된다.
        } else { // 나머지의 경우에는 idx를 1씩 증가시킨다.
